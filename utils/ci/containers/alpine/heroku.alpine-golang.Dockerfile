...
COPY alpine.golang.hugo-extended.setup.sh .
# we need extended, and nodejs for sass!
RUN chmod +x ./alpine.golang.hugo-extended.setup.sh && ./alpine.golang.hugo-extended.setup.sh
...
RUN mkdir -p /pokusio.io/hugo/src/
COPY . /pokusio.io/hugo/src/
COPY .git /pokusio.io/hugo/src/
RUN ls -allh /pokusio.io/hugo/src/
RUN export PATH=$PATH:/usr/local/go/bin && cd /pokusio.io/hugo/src/ && hugo -b "${HUGO_BASE_URL}"
...
LABEL io.pokus.daymood="https://www.youtube.com/watch?v=v-JsqKlVVGk&list=RDv-JsqKlVVGk"
LABEL io.pokus.oci.base.image="httpd:${HTTPD_OCI_IMAGE_TAG}"
...
RUN mkdir -p /pokusio.io/retrieved_hugo_build
COPY --from=hugo_build /pokusio.io/hugo/src/public /pokusio.io/retrieved_hugo_build
#   fRench do it, Let(')s do it, Let's...
RUN cp -fR /pokusio.io/retrieved_hugo_build/* /usr/local/apache2/htdocs
...
COPY heroku.apache.start.sh /pokusio.io
COPY heroku.container.healthcheck.sh /pokusio.io
RUN chmod +x /pokusio.io/*.sh

#
# healthcheck:
#   test: ["CMD", "/pokusio.io/website.healthcheck.sh"]
#   interval: 5s
#   timeout: 10s
#   retries: 30
#   start_period: 60s
...
ENTRYPOINT ["/pokusio.io/heroku.apache.start.sh"]
